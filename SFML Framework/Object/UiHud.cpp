...
{
	float textSize = 50.f;
	sf::Font& font = FONT_MGR.Get("fonts/H2GTRE.ttf");
...
{
	std::wstring temp = STRING_TABLE->Get("UiScore",Variables::currentLang);
	textScore.setString(temp + std::to_wstring(s));
	Utils::SetOrigin(textScore, Origins::TL);
}

void UiHud::SetHiScore(int s)
{
	std::wstring temp = STRING_TABLE->Get("UiHiScore", Variables::currentLang);
	textHighScore.setString(temp + std::to_wstring(s));
	Utils::SetOrigin(textHighScore, Origins::TR);
}
...
{
	std::wstring temp = STRING_TABLE->Get("UiZombieCnt", Variables::currentLang);
	textZombieCount.setString(temp + std::to_wstring(count));
	Utils::SetOrigin(textZombieCount, Origins::BR);
}
